    ...
    return global.pdfjsLib.getDocument({ data: bytes, isEvalSupported: false }).promise
      .then(function (pdf) {
        ...
        return chain.then(function () {
          // ========== PDF 解析模式（v2.0.27：逐页高清图成品版 0丢失0偏差）==========
          // ▶ 默认（实际使用，v2.0.27）：强制所有 PDF 走逐页 Canvas 高清图渲染，
          //    每页最大宽度 1100px、JPEG 质量 0.82，保证封面大标题、页眉页脚、小标题、
          //    粗体下划线、分栏、表格全部像素级还原。
          // ▶ 切换到文字可复制版（GitHub留档字迹版）：把下面一行注释掉，
          //    反注释上面 6 行字迹版代码即可。
          return buildPdfImageHtml(pages);
          //  字迹版（启用方式：注释上一行，反注释下一段）：
