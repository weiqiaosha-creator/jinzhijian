...
(function () {
  ...
  panel.innerHTML =
    ...
    '<div id="jz-drop" style="border:2px dashed #c4ccd6;border-radius:8px;padding:16px;text-align:center;color:#8a93a0;cursor:pointer;">拖入 .md/.docx/.pptx/.pdf<br><span style="font-size:11px;">.doc 转 .docx；PDF 文字件可复制、扫描件转图；支持 GIF</span></div>' +
    '<input id="jz-file" type="file" multiple accept=".md,.markdown,.txt,.docx,.pptx,.doc,.gif,.pdf" style="display:none;">' +
    ...
    '<button id="jz-upload" title="认证号：把已转换内容上传到公众号草稿/素材库" style="flex:1;padding:8px;border:none;border-radius:7px;background:#0b7285;color:#fff;font-weight:600;cursor:pointer;font-size:12px;">⬆ 上传草稿</button>' +
  ...
  // ======================== 模块级 toast（不依赖 initBusiness） ========================
  function toastModule(msg, color) {
    try {
      var c = color || '#00A86B';
  ...
  // ======================== 早期事件委托：插入编辑器 ========================
  // v2.0.22：用面板级事件委托在面板创建后立即绑定「插入编辑器」，
  // 不受 initBusiness 中途抛错的影响。实际能力由 initBusiness 填充到 jzApi。
  var jzApi = { ready: false };
  panel.addEventListener('click', function (e) {
    var el = e.target;
    while (el && el !== panel && el.id !== 'jz-insert' && el.id !== 'jz-upload') el = el.parentNode;
    if (!el || el === panel) return;
    e.preventDefault();
    e.stopPropagation();
    var btnId = el.id;
    var hint = '（请确认已进入公众号「图文编辑」页面，并打开一篇图文草稿）';
    if (!jzApi.ready) {
      toastModule('⚠️ 功能未就绪：依赖初始化失败', '#d9480f');
      status('功能未就绪：converter/themes 依赖未就绪或初始化抛错。请打开 F12 Console 看红色报错，并重载扩展、强制刷新（Ctrl+Shift+R）。', true);
      return;
    }
    // 「上传草稿」走认证号 API，直接创建公众号草稿，不依赖打开编辑器
    if (btnId === 'jz-upload') {
      if (typeof jzApi.upload === 'function') { jzApi.upload(); }
      else { toastModule('⚠️ 上传功能未就绪', '#d9480f'); status('上传功能未就绪。', true); }
      return;
    }
    var html = '';
    try { html = jzApi.combined(); } catch (e0) { html = ''; }
    if (!html) {
      var errs = (jzApi.errors || function () { return []; })();
      if (errs.length) {
        // 直接把首个具体错误带上，避免用户只看到泛化的「转换失败」
        toastModule('⚠️ ' + errs.length + ' 个文件转换失败：' + errs[0].name + ' → ' + errs[0].error, '#d9480f');
        status('转换失败：' + errs.map(function (x) { return x.name + ' → ' + x.error; }).join(' | '), true);
        console.error('[瑾之笺] 转换失败明细:', errs);
      } else {
        toastModule('⚠️ 暂无内容可插入', '#d9480f');
        status('没有可插入的内容：请先拖入 md/docx/pptx/pdf 文件，或点击「抓取」采集文章。', true);
      }
      return;
    }
    try {
      var themeLabel = jzApi.themeLabel();
      var ed = jzApi.findEditor();
      if (!ed) { toastModule('⚠️ 未找到正文编辑器', '#d9480f'); status('未找到正文编辑器' + hint, true); return; }
      var r = jzApi.insertStyledDom(ed, html, jzApi.themeName());
      if (r.ok) {
        toastModule('✅ 已插入（主题：' + themeLabel + '）', '#00A86B');
        status('已插入正文编辑器（主题：' + themeLabel + '）——DOM 物理迁移，样式不会被微信清洗。');
      } else {
        toastModule('⚠️ ' + (r.reason || '插入失败'), '#d9480f');
        status('插入失败：' + (r.reason || '未知错误') + hint, true);
      }
    } catch (e_ins) {
      console.error('[瑾之笺] 插入异常:', e_ins);
      toastModule('❌ 插入异常：' + (e_ins && e_ins.message || e_ins), '#d9480f');
      status('插入异常：' + (e_ins && e_ins.message || e_ins) + hint, true);
    }
  });
    ...
    var timer = setInterval(function () {
      ...
      if (st.markdownit && st.mammoth && st.JSZip && st.QSWechatCommon && st.QSThemes && st.QSThemeOrder && st.QSConverter) {
        ...
        try { initBusiness(); updateDiagnose(st); }
          ...
          try { toast('❌ 业务初始化失败：' + (e && e.message) + '（请打开 F12 Console 看红色报错）', '#d9480f'); } catch (e2) {}
          status('初始化失败：业务绑定出错（' + (e && e.message) + '），已尝试尽量保留「插入」功能，请截图本条提示。', true);
        }
  ...
  function initBusiness() {
    ...
    // v2.0.23：页面级拖入——在公众号页面的任意位置拖入受支持的文档也会自动进面板转换。
    // 原先只在面板右上角那块小虚线区域接住拖拽，用户把 PDF 拖到正文/页面其它地方时
    // 什么都没发生，点「插入编辑器」自然就是「暂无可插入内容」。
    // 这里只拦截 md/txt/docx/pptx/pdf/doc 这些文档类型；图片仍走微信原生拖入，不抢占正文插图能力。
    var PAGE_DROP_OK = /\.(md|markdown|txt|docx|pptx|pdf|doc)($|\?)/i;
    function pageHasSupportedFile(dt) {
      if (!dt || !dt.files || !dt.files.length) return false;
      var hit = false;
      Array.prototype.forEach.call(dt.files, function (f) {
        if (PAGE_DROP_OK.test((f.name || ''))) hit = true;
      });
      return hit;
    }
    document.addEventListener('dragover', function (e) {
      // 只有拖的是受支持文档时才阻止默认（允许放置）；图片/普通拖动放行给页面本身
      if (pageHasSupportedFile(e.dataTransfer)) e.preventDefault();
    }, true);
    document.addEventListener('drop', function (e) {
      if (!pageHasSupportedFile(e.dataTransfer)) return;
      e.preventDefault();
      e.stopPropagation();
      var chosen = [];
      Array.prototype.forEach.call(e.dataTransfer.files, function (f) {
        if (PAGE_DROP_OK.test((f.name || ''))) chosen.push(f);
      });
      if (!chosen.length) return;
      toastModule('已收到 ' + chosen.length + ' 个文档，正在转换…（看右侧面板）', '#00A86B');
      addFiles(chosen);
    ...
    function syncArticleTitle(wrap) {
      ...
      if (!t) return;
      // 先同步尝试一次（大多数情况下标题框已经在 DOM 中），若失败再做最多 2 秒的 4 次重试，
      // 以兼容公众号后台某些版本的「标题框挂载延迟」。
      var okNow = fillTitle(t);
      if (okNow) return;
      var tries = 0;
      var maxTries = 4;
      var iv = setInterval(function () {
        tries++;
        if (fillTitle(t) || tries >= maxTries) clearInterval(iv);
      }, 500);
    }

    // 返回是否「已经填进去」（方便调用方停止重试）。
    function fillTitle(title) {
      if (!title) return false;
      var input = null;
      var triedInputs = [];

      // ★ 优先定位公众号「新版编辑器」的可见标题框：
      // 新版的标题框 = 一个 .ProseMirror 可编辑 div，外层容器 class 通常含 title-editor__input；
      // 它没有 placeholder 属性，旧的 input/textarea 选择器匹配不到（只会匹配到隐藏的
      // textarea#title，填入后可见标题框并不会更新——这正是「标题一直同步不上」的根因）。
      // 所以必须先于标准选择器命中它。
      var pm = findProseMirrorTitle();
      if (pm && isTitleInputEmpty(pm)) input = pm;

      // 候选选择器：覆盖公众号后台历史至今出现的几种写法（新旧两种编辑器 + 旧后台）。
      // 按「匹配精准度」排序：先找带「标题」placeholder 的 input；再找常见 id/class。
      var cand = [
        ...
        '.we-desktop-form__input[placeholder*="标题"]',
        '#mp-editor-toolbar input[placeholder*="标题"]',
        'div.edui-editor-textarea[title*="标题"]',
        'div.weui-desktop-form__input[contenteditable="true"][placeholder*="标题"]'
      ];
      for (var i = 0; i < cand.length; i++) {
        var c = null;
        try { c = document.querySelector(cand[i]); } catch (e_s) { c = null; }
        if (!c) continue;
        if (!isTitleInputEmpty(c)) { triedInputs.push(c); continue; } // 已填过值：跳过，继续找下一个
        input = c; break;
      }

      // 兜底：若标准选择器都没命中（或者都命中了但元素已被用户填过），枚举所有可见
      // input / contenteditable，用 placeholder/aria-label/空值+位置打分找「最像标题框的」。
      if (!input) {
        var fallback = findTitleInputHeuristic(triedInputs);
        if (fallback && isTitleInputEmpty(fallback)) input = fallback;
      }

      if (!input) return false;

      // contenteditable 用 textContent；原生 input 用 value。
      var isContentEditable = false;
      try {
        if (input.getAttribute && input.getAttribute('contenteditable') === 'true') isContentEditable = true;
        if (input.isContentEditable) isContentEditable = true;
      } catch (e_ce) { /* ignore */ }

      try {
        if (isContentEditable) {
          // ProseMirror 专属注入：必须先 focus，再通过 execCommand('insertText') 让框架认账，
          // 直接改 textContent 进不了 ProseMirror 的文档模型，画面不会更新。
          try {
            input.focus();
            var sel = window.getSelection && window.getSelection();
            if (sel && sel.selectAllChildren) sel.selectAllChildren(input);
            ...
            try { input.textContent = title; } catch (e_ceInj2) { /* ignore */ }
          }
          try {
            input.dispatchEvent(new Event('input', { bubbles: true }));
            input.dispatchEvent(new Event('change', { bubbles: true }));
          } catch (e_ed2) { /* ignore */ }
          // 同步隐藏的同名底层字段（新编辑器通常还有一个隐藏的 textarea#title 供后端读取）
          try {
            var hd = document.querySelector('#title');
            if (hd && !hd.isContentEditable && hd.value !== undefined) hd.value = title;
          } catch (e_hd) { /* ignore */ }
          return true;
        }

        // React / Vue 控制的 input：必须先绕过 setter 再赋值，否则 value 会被下次 diff 刷回来。
        var prototype = window.HTMLInputElement && HTMLInputElement.prototype;
        var descriptor = prototype && Object.getOwnPropertyDescriptor(prototype, 'value');
        var setter = descriptor && descriptor.set;
        if (setter) {
          try { setter.call(input, title); } catch (e_st) { input.value = title; }
        } else {
          input.value = title;
        }
        try { input.dispatchEvent(new Event('input', { bubbles: true })); } catch (e_ev) {}
        try { input.dispatchEvent(new Event('change', { bubbles: true })); } catch (e_e2) {}
        try { input.dispatchEvent(new Event('blur', { bubbles: true })); } catch (e_e3) {}
        // 额外兼容：部分框架监听 propertychange
        try {
        ...
        // 最后兜底：直接赋值
        try { input.value = title; return true; } catch (e_last) { return false; }
      }
    }

    // 定位公众号「新版编辑器」的可见标题框（.ProseMirror）。
    // 判定准则：contenteditable、当前为空、位于页面上部、宽度够宽、高度像输入框（15~80px），
    // 且祖先容器 class 含 title 者为最优。这样能精准跳过正文/作者等其它 ProseMirror。
    function findProseMirrorTitle() {
      try {
        var all = document.querySelectorAll('.ProseMirror[contenteditable="true"], .ProseMirror[contenteditable=""]');
        var best = null, bestScore = 0;
        for (var i = 0; i < all.length; i++) {
          var e = all[i];
          if (!e || !isTitleInputEmpty(e)) continue; // 已填过（或元素无效）：不覆盖
          var r;
          try { r = e.getBoundingClientRect(); } catch (e_r) { continue; }
          var w = r.width, h = r.height, top = r.top;
          if (w < 200 || h < 15 || h > 80) continue;  // 尺寸不像标题框
          if (top > 500) continue;                    // 必须在页面上部
          var s = 0;
          if (top < 260) s += 5;                       // 顶部强加分
          var anc = e;
          for (var d = 0; d < 6 && anc; d++) {
            anc = anc.parentNode;
            if (anc && anc.nodeType === 1 && /title/i.test(anc.className || '')) { s += 10; break; }
          }
    ...
    // 判断一个候选元素是否「还没被用户填过标题」。
    // ⚠️ 必须剔除零宽/不可见字符：ProseMirror 空文档里不是空文本，而是零宽字符
    //（\u200b）或内嵌换行。若只 .trim() 去普通空白，会把「空的可见标题框」误判成
    //「已填过」而跳过——这正是 v2.0.18 之前标题始终不同步的深层根因。
    function isTitleInputEmpty(el) {
      if (!el) return false;
      var isContentEditable = false;
      try {
        if (el.getAttribute && el.getAttribute('contenteditable') === 'true') isContentEditable = true;
        if (el.isContentEditable) isContentEditable = true;
      } catch (e_ce) { /* ignore */ }
      var current = '';
      if (isContentEditable) {
        try { current = (el.innerText !== undefined ? el.innerText : (el.textContent || '')); } catch (e1) { current = ''; }
      } else {
        try { current = (el.value !== undefined ? el.value : ''); } catch (e2) { current = ''; }
      }
      return isEmptyAfterInvisible(current);
    }

    // 剔除所有零宽/不可见/标点控制字符与空白后仍为空，才算「真的空」。
    function isEmptyAfterInvisible(s) {
      if (!s) return true;
      try {
        return s
          .replace(/[\u200b-\u200f\u2060-\u2069\ufeff\u00ad\u061c\u180e]/g, '')
    ...
    // 启发式兜底查找标题框：遍历 <input> + [contenteditable]，按文本/标签打分。
    // （可选传入「标准选择器已试过的非空元素」，避免重复选到它们。）
    function findTitleInputHeuristic(excludeList) {
      try {
        var scoreList = [];
        function score(el) {
          var s = 0;
          function addIfHit(str, w) { if (typeof str === 'string' && /标题|文章标题/.test(str)) s += w; }
          function minusIfHit(str, w) { if (typeof str === 'string' && /作者|author/i.test(str)) s -= w; }
          try { addIfHit(el.getAttribute && el.getAttribute('placeholder'), 10); } catch (e0) {}
          try { minusIfHit(el.getAttribute && el.getAttribute('placeholder'), 14); } catch (e0b) {} // 明显是作者框：强扣分
          try { addIfHit(el.getAttribute && el.getAttribute('aria-label'), 8); } catch (e1) {}
          try { minusIfHit(el.getAttribute && el.getAttribute('aria-label'), 12); } catch (e1b) {}
          try { addIfHit(el.getAttribute && el.getAttribute('data-name'), 8); } catch (e2) {}
          try { addIfHit(el.getAttribute && el.getAttribute('title'), 6); } catch (e3) {}
          try { addIfHit(el.name, 6); } catch (e4) {}
          try { minusIfHit(el.name, 12); } catch (e4b) {}
          try {
            if (typeof el.id === 'string') {
              if (/文章标题|title/i.test(el.id)) s += 6; // id 直接含 title/article_title：强加分
              else if (/作者|author/i.test(el.id)) s -= 14; // 明显是作者框：强扣分
            }
          } catch (e5) {}
          try {
            if (el.className && typeof el.className === 'string') {
              if (/title/i.test(el.className)) s += 5;
              else if (/作者|author/i.test(el.className)) s -= 12;
            }
          } catch (e6) {}
          // 可见性：必须在页面上方（y < 600），且尺寸明显是输入框。
          // 注意：如果元素是绝对定位子节点且父容器高度为 0，getBoundingClientRect 可能返回 0 尺寸，
          // 这时需回退到 CSS 样式判断，避免把「其实可见」的 input 误扣分。
          try {
            var r = el.getBoundingClientRect();
            var w = r.width, hgt = r.height, top = r.top;
            if ((w < 10 || hgt < 10) && el.style) {
              var sW = el.style.width;
              var sH = el.style.height;
              if (/px/.test(sW)) { w = parseFloat(sW) || 0; }
              else if (sW && /%/.test(sW)) { w = 300; } // 百分比宽度：认为有足够宽度
              if (/px/.test(sH)) { hgt = parseFloat(sH) || 0; }
              else if (sH && /%/.test(sH)) { hgt = 30; }
            }
            if (w < 40 || hgt < 10) s -= 20;
            else if (top < 80 && hgt > 20 && w > 200) s += 8;    // 页面最顶栏（通常就是标题框）强加分
            else if (top < 250 && hgt > 20 && w > 200) s += 5;   // 顶部偏上：仍优先
            else if (top < 600 && hgt > 20 && w > 200) s += 3;
            else if (top >= 600) s -= 10;
          } catch (e_r) { /* ignore */ }
          // 空值加分（用户还没填过，更可能是我们要找的）
          try {
            if (el.value !== undefined && !(el.value || '').trim()) s += 2;
          } catch (e_val) { /* ignore */ }
          return s;
        }
        function isExcluded(el) {
          if (!excludeList || !excludeList.length) return false;
          for (var k = 0; k < excludeList.length; k++) if (excludeList[k] === el) return true;
          return false;
        }
        function collect(sel) {
          try {
            ...
            for (var i = 0; i < list.length; i++) {
              if (isExcluded(list[i])) continue;
              if (!isTitleInputEmpty(list[i])) continue; // 已填过值：直接忽略
              scoreList.push({ el: list[i], s: score(list[i]) });
            }
          } catch (e_c) { /* ignore */ }
        }
        collect('input[type="text"]');
        collect('input:not([type])');
        collect('textarea');
        collect('[contenteditable="true"]');
        scoreList.sort(function (a, b) { return b.s - a.s; });
        if (scoreList.length && scoreList[0].s >= 6) return scoreList[0].el;
        return null;
      } catch (e_h) { return null; }
    }
    ...
    // 「插入编辑器」已由面板级早期事件委托接管（见模块顶部 jzApi）。
    // 这里仅把实际能力填充进 jzApi，供委托处理器调用；若 initBusiness 走到这之前抛错，
    // jzApi.ready 仍为 false，点击时也会给出「功能未就绪」的可视提示，绝不静默无反应。
    jzApi.combined = combined;
    jzApi.findEditor = findEditor;
    jzApi.insertStyledDom = insertStyledDom;
    jzApi.themeName = function () { return themeName; };
    jzApi.themeLabel = function () {
      return (themeSel.options[themeSel.selectedIndex] || {}).textContent || themeName;
    };
    jzApi.errors = function () {
      return files.filter(function (i) { return i.error; })
        .map(function (i) { return { name: i.file.name, error: i.error }; });
    };
    jzApi.upload = function () {
      if (isPersonal()) {
        toastModule('⚠️ 个人号无 API 草稿能力', '#d9480f');
        status('个人号不能用 API 上传草稿。请改用「插入编辑器」把内容填到当前打开的公众号编写界面，再手动保存草稿/发表。', true);
        return;
      }
      var title = panel.querySelector('#jz-title').value || (files[0] && files[0].title) || '未命名文章';
      var html = '';
      try { html = combined(); } catch (e0) { html = ''; }
      if (!html) {
        var uerrs = (jzApi.errors || function () { return []; })();
        if (uerrs.length) toastModule('⚠️ ' + uerrs.length + ' 个文件转换失败，无法上传', '#d9480f');
        else toastModule('⚠️ 暂无内容可上传', '#d9480f');
        status(!html ? '没有可上传的内容：请先拖入/抓取并成功转换。' : '', !html);
        return;
      }
      status('读取凭证，准备上传到公众号草稿…');
      chrome.storage.local.get(['jz_appid', 'jz_secret'], function (cfg) {
        if (!cfg.jz_appid || !cfg.jz_secret) {
          toastModule('⚠️ 未配置 AppID/Secret', '#d9480f');
          status('请点击浏览器右上角的扩展图标，在弹出的窗口填入认证号 AppID/Secret 并保存，再回来点「上传草稿」。', true);
          return;
        }
        status('正在上传正文图片到素材库并创建草稿（可能需要几秒）…');
        chrome.runtime.sendMessage({
          type: 'publish',
          payload: { title: title, html: html, scheduleTime: '', appid: cfg.jz_appid, secret: cfg.jz_secret }
        }, function (resp) {
          if (resp && resp.ok) {
            toastModule(resp.message, '#00A86B');
            status('上传成功：' + (resp.message || '已生成草稿'));
          } else {
            toastModule('❌ ' + ((resp && resp.error) || '上传失败'), '#d9480f');
            status('上传失败：' + ((resp && resp.error) || '未知错误'), true);
            chrome.runtime.sendMessage({ type: 'notify', text: '瑾之笺：上传草稿失败 — ' + ((resp && resp.error) || '未知') });
          }
        });
      });
    };
