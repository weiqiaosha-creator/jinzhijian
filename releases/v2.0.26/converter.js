    ...
    return global.pdfjsLib.getDocument({ data: bytes, isEvalSupported: false }).promise
      .then(function (pdf) {
        ...
        return chain.then(function () {
          // ========== PDF 解析模式（v2.0.26：字迹版）==========
          // ▶ 默认（v2.0.26）：文字可复制版 —— 通过 pdf.js getTextContent 提取文字，
          //    按字符数×频次加权估算正文字号基线，建立 h1/h2/h3/p 四档离散分档，
          //    重建 HTML。扫描件/纯图片 PDF 若文字量低于阈值则自动兜底逐页高清图。
          // ▶ 切换方法（切到逐页高清图 0 丢失版）：把下面 6 行字迹版代码注释掉，
          //    反注释最后一行 `return buildPdfImageHtml(pages);` 即可。
