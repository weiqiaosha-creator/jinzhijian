...
(function () {
  ...
  function initBusiness() {
    ...
    panel.querySelector('#jz-insert').onclick = function () {
      ...
      if (!html) { toast('⚠️ 暂无内容可插入', '#d9480f'); status('没有可插入的内容：请先拖入 md/docx/pptx/pdf 文件，或点击「抓取」采集文章。', true); return; }
      try {
        var themeLabel = (themeSel.options[themeSel.selectedIndex] || {}).textContent || themeName;
        var ed = findEditor();
        if (!ed) { toast('⚠️ 未找到正文编辑器', '#d9480f'); status('未找到正文编辑器' + hint, true); return; }
        ...
        toast('❌ 插入异常：' + (e_ins && e_ins.message || e_ins), '#d9480f');
