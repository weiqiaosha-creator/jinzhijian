      ...
      for (var idx = 0; idx < files.length; idx++) {
        (function (i, fixedIdx) { // 闭包捕获固定索引
          ...
          var label = (i.degraded ? '🚧 降级 ' : '') + i.file.name + '：' +
            (i.error ? (err ? '⚠ ' + i.error.split('\n').slice(0, 2).join(' | ') : '失败 ' + i.error) : (i.html ? '已转换' : '转换中'));
          d.textContent = label;
          d.style.color = i.degraded ? '#e08020' : (i.error ? '#e03131' : (i.html ? '#1a9e57' : '#8a93a0'));
          if (err || i.error) d.style.cursor = 'pointer';
          d.onclick = function () { if (i.error) alert(i.error); };
          wrap.appendChild(d);
